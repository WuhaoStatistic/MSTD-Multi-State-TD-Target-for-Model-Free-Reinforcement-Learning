...
def main(seed):
    ...
    p = '{}'.format('GPDDPG_' + BriefEnvName[env_index] + '_') + timenow
    ...
    timenow = str(datetime.now()).split('.')[0].replace(':', '_')
    p = '{}'.format('MSDDPG action generated ' + BriefEnvName[env_index] + '_') + timenow
    wdir = os.path.join('runs', BriefEnvName[env_index], 'MSDDPG action generated {}steps'.format(predictive_step))
